# visual-state-semantics Specification

## Purpose
Define a coherent visual language for dense TUI screens so form hierarchy, state semantics, and surface layering stay readable and consistent across the application.

## Requirements
### Requirement: Dense forms establish a scannable information hierarchy
The TUI SHALL visually distinguish field labels, editable values, metadata badges, help text, and validation messaging so dense forms can be scanned without reading each line in order.
...
The TUI SHALL assign stable semantic roles to focus, selection, passive metadata, inherited or implicit state, success, warning-like state, and error feedback so different meanings do not rely on the same accent treatment.
...
- **WHEN** the app renders the outer background, sidebar, content panel, input surface, and preview surface in any supported theme preset
- **THEN** neighboring layers remain visually separable without relying on border color alone

### Requirement: Focused elements are unmistakable during keyboard navigation
The TUI SHALL make the currently focused interactive element visually distinct from merely selected, hovered, opened, or visible elements across the sidebar, form, and preview-adjacent controls.

#### Scenario: Sidebar contains an active row while focus is elsewhere
- **WHEN** the selected command row remains active but focus is in the form or search field
- **THEN** the active sidebar row remains identifiable
- **AND** the focused surface is still visually stronger than the unfocused active row

#### Scenario: User navigates through a long form with the keyboard
- **WHEN** focus moves between fields in the form
- **THEN** the focused field uses a stronger treatment than neighboring unfocused fields
- **AND** that treatment remains distinguishable from invalid-only styling

### Requirement: Surface chrome follows a shared visual language
The TUI SHALL use a consistent chrome vocabulary for major surfaces so sidebar, workspace, preview, overlays, and compact action areas feel related even when their layout differs.
...
- **THEN** their borders, titles, layering, or background treatments read as belonging to one design system
- **AND** differences in importance are expressed intentionally rather than by widget-local accident

...
The TUI SHALL present control affordances and inline hints that match each widget's real interaction model.
...
- **AND** the UI explains that editing will create a local override for the selected command path
...
The TUI SHALL visually distinguish primary actions, passive hints, validation summaries, success feedback, and error feedback.
...
- **WHEN** the UI renders primary actions, inherited badges, validation summaries, or toasts in any supported theme
